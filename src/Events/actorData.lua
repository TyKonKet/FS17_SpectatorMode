...
function ActorDataEvent:new(actorName, tx, ty, tz, rx, ry, rz, rw)
...
function ActorDataEvent:run(connection)
    ...
    if g_spectatorMode ~= nil then
        g_spectatorMode:actorDataEventF(self.actorName, self.tx, self.ty, self.tz, self.rx, self.ry, self.rz, self.rw);
    end
end
